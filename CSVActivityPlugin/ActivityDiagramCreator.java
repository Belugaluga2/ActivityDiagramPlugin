...
public class ActivityDiagramCreator {
    ...
    private void createSubactionDiagram(Project project,
        ...
        for (ActivityData row : subRows) {
            String actor = actorName(row.getActor());
            ActivityPartition lane = partitions.get(actor);
            if (lane == null) continue;

            for (Element e : parentNode.getOwnedElement()) {
                if (e instanceof ActivityNode n && row.getName().equals(n.getName())) {
                    n.getInPartition().clear();     // remove any stale link
                    lane.getNode().add(n);          // attach to the fresh lane
                }
            }
        }

        /* Also park Start/End in the first lane so their shapes match 
        if (!partitions.isEmpty()) {
            ActivityPartition firstLane = partitions.values().iterator().next();
            for (Element e : parentNode.getOwnedElement()) {
                if (e instanceof InitialNode || e instanceof ActivityFinalNode) {
                    ActivityNode n = (ActivityNode) e;
                    n.getInPartition().clear();
                    firstLane.getNode().add(n);
                }
            }
        }
		*/
        ...
        if (startNode == null) {
            ...
            if (!partitions.isEmpty()) {
                partitions.values().iterator().next().getNode().add(startNode);
            }
        }
        ...
        if (endNode == null) {
            ...
            // Add to first partition
            if (!partitions.isEmpty()) {
                partitions.values().iterator().next().getNode().add(endNode);
            }
        }
        ...
        buildAndPlaceSwimlanes(subDpe, partitions,
        ...
        /* Layout the nodes */
        SubdiagramGridLayouter.layoutNodeList(nodesToLayout, subDpe, partitions, START_Y, Y_STEP);
    ...
    /**
     * Always create fresh partitions for each actor in subRows.
     */
    private Map<String,ActivityPartition> createOrReusePartitions(Project project,
                                                                  Element context,
                                                                  List<ActivityData> subRows)
            throws ReadOnlyElementException {

        Activity owner = findOwningActivity(context);
        if (owner == null)
            throw new IllegalStateException("Cannot resolve owning Activity.");
        ...
        // Collect distinct actor names
        Set<String> actors = new LinkedHashSet<>();
        for (ActivityData d : subRows) {
            actors.add(actorName(d.getActor()));
        }

        // Create a brand‑new partition for each actor
        Map<String,ActivityPartition> out = new LinkedHashMap<>();
        for (String actor : actors) {
            ActivityPartition p = f.createActivityPartitionInstance();
            p.setName(actor);
            ...
            // Attach it to the owning Activity
            mgr.addElement(p, owner);
            owner.getPartition().add(p);

            // Apply SysML stereotype if available
            if (stereo != null) {
                StereotypesHelper.addStereotype(p, stereo);
            }
